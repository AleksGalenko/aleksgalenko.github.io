                            ...
                            <table class="product-table">
                                <tr>
                                    <td rowspan="2" class="product-image">
                                        <img src="${product.image}" alt="Product Image" width="35">
                                    </td>
...
function renderPOS() {
    ...
    selectedProducts.forEach(productId => {
        const totalInput = document.querySelector(`.totalInput[data-index="${productId}"]`);
...
document.getElementById('product-list').addEventListener('click', (event) => {
    if (event.target.classList.contains('addButton')) {
        ...
        const quantityInput = document.querySelector(`.quantityInput[data-index="${skuId}"]`);
...
function updatePOSIndicator() {
    ...
    document.getElementById('total-pos-indicator').textContent = `Total POS ${posData.length}`;
}
